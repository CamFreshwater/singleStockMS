...
Large allocations to mixed-, rather than single-stock fisheries, resulted in reductions in aggregate conservation performance, including median return abundance, median spawner abundance, and the mean proportion of stocks above their conservation benchmark (Fig. 1, Appendix S2). Conversely median aggregate catch and catch stability were maximized at intermediate allocations to mixed-stock fisheries (Fig. 1, Appendix S2). Impacts on conservation- and catch-based metrics were driven by differences in realized exploitation rates, which increased with mixed-stock fishery allocations (Appendix S2).
...
```{r altTO, warnings=FALSE, messages=FALSE, fig.cap="The impact of an alternative mixed stock harvest control rule (b) and en route mortality prior to single-stock fisheries (c), on trade-offs between aggregate return abundance and catch. Points represent medians and whiskers represent 90th percentile intervals among Monte Carlo trials. Note that panel (a) is the same as Fig. 1a and is presented here to facilitate comparison."}
altOMDat <- agDat %>% 
  mutate(om = case_when(
