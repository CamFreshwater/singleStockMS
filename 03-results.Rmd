...
Large allocations to mixed-stock, rather than single-stock, fisheries resulted in reductions in aggregate conservation performance, including median (among simulation years) return abundance, median spawner abundance, and the mean proportion of stocks above their conservation benchmark (Fig. 1a-c). Conversely median aggregate catch and catch stability were maximized at intermediate allocations to mixed-stock fisheries (Fig. 1d,e). Impacts on conservation- and catch-based metrics were driven by differences in realized exploitation rates (Fig. 1f), which increased with mixed-stock fishery allocations.
...
The timing of en route mortality also altered trade-offs among fishery allocations, with fully single-stock fisheries exhibiting the strongest declines in performance. For example, when en route mortality occurred before single-stock fisheries, median aggregate return abundance in the fully single-stock maangement procedure declined by `r sing_rec$perc_dec` relative to the reference operating model (Fig. \@ref(fig:altTO)a,c). Similarly, median aggregate catch in fully single-stock fisheries declined by `r sing_catch$perc_dec` (Fig. \@ref(fig:altTO)a,c). Conversely, management procedures with intermediate fishery allocations or fully mixed-stock fisheries were less severely impacted.
