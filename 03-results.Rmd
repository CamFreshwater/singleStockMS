...
The final model adjustment we considered was shifting the timing of en route mortality to precede single-stock fishery harvest, representing a fully terminal harvest scenario, which resulted in strong declines in performance. For example, median aggregate return abundance in the fully single-stock management procedure declined by `r sing_rec$perc_dec` relative to the reference operating model and median aggregate catch declined by `r sing_catch$perc_dec` (Fig. \@ref(fig:altTO)a,c). Conversely, this adjustment had only modest impacts on management procedures with intermediate fishery allocations or fully mixed-stock fisheries.
