...
```{r hcrTO, echo=FALSE, warning=FALSE, warnings=FALSE, messages=FALSE, fig.cap="Figure S2.2 Comparison of generic multi-stock harvest control rule and a more precautionary total allowable mortality (TAM) rule. Points represent medians."}
refDat2 <- agDat %>% 
  filter(om == "ref") %>% 
  mutate(hcr = mp)
...
plotAgTradeoff(refDat2 %>% select(-labelledVar), consVar = "medRecRY", 
               ...
               showUncertainty = FALSE, xLab = "Median Aggregate Catch", 
