...
listOfPackages <- c("plyr", "here", "parallel", "doParallel", "foreach", 
                    ...
                    "sensitivity", "mvtnorm", "devtools", "samSim")
...
simPar <- read.csv(here("data/fraserMPInputs_varyMixPpnTEST.csv"), stringsAsFactors = F)
...
ricPars <- read.csv(here("data/rickerMCMCPars.csv"), stringsAsFactors = F)
larkPars <- read.csv(here("data/larkinMCMCPars.csv"), stringsAsFactors = F)
...
simParTrim <- simPar
# simParTrim <- subset(simPar, scenario == "refNoOU")
                     #| scenario == "constrain" |
...
# trade off plots
consVec <- c("medEarlyS", "medSpawners", "ppnYrsLower")
yLabVec <- c( "Median Early Spawner Abundance", "Median Spawner Abundance",
             "Ppn Years Above Low BM")
catchVec <- c("medEarlyTotalCatch", "medCatch", "medCatch")
xLabVec <- c("Median Early Catch", "Median Catch", "Median Catch")
...
pdf(here("figs/test/Fig1_CUTradeoffs.pdf"), 
...
pdf(here("figs/test/Fig2_CUDotplots.pdf"), 
    height = 6.5,
    width = 9.5)
...
agDat <- buildDataAgg(dirNames, agVars =  c("medSpawners", "medCatch", 
                                             ...
                                             "medSpawnersLate", "medCatchLate"), 
                       keyVarName = "ppnMixed")

# trade off plots
consVec <- c("medSpawnersEarly", "medSpawnersLate", "medSpawners", "ppnCULower")
yLabVec <- c("Median Early Spawner Abundance", "Median Late Spawner Abundance", 
             "Median Spawner Abundance", "Prop CU Above LB")
catchVec <- c("medCatchEarly", "medCatchLate", "medCatch", "medCatch")
xLabVec <- c("Median Early Catch", "Median Late Catch", "Median Catch", 
             "Median Catch")
...
pdf(here("figs/test/Fig3_AgTradeoffs.pdf"), 
...
pdf(here("figs/test/Fig4_AgDotPlot.pdf"), 
...
pdf(here("figs/test/Fig5_AgRadarPlot_moveTAC.pdf"), 
    height = 6, width = 7.5)
...
### Pull trial data and look at spawner trends through time
singDat <- readRDS(here("outputs/simData/refNoOU_sockeye/ref/ref_static0.0TAM_synchArrays.RData"))
splitDat <- readRDS(here("outputs/simData/refNoOU_sockeye/ref/ref_static0.5TAM_synchArrays.RData"))
mixDat <- readRDS(here("outputs/simData/refNoOU_sockeye/ref/ref_static1.0TAM_synchArrays.RData"))


plot(singDat$S[60:100, 9, 1], type = "l", ylab = "S")
lines(mixDat$S[60:100, 9, 1], col = "red")
lines(splitDat$S[60:100, 9, 1], col = "blue")
plot(singDat$totCatch[60:100, 9, 1], type = "l", ylab = "catch")
lines(mixDat$totCatch[60:100, 9, 1], col = "red")
lines(splitDat$totCatch[60:100, 9, 1], col = "blue")

singDat2 <- agList[[1]][["Ag Catch"]]
